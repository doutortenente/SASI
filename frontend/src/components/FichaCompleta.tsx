  ...
  const handleSyncToPatientSummary = useCallback(async () => {
    ...
    try {
      ...
      const metasTxt = condutasSistemasDraft
        .filter(c => c.meta)
        .slice(0, 5)
        .map(c => `• ${c.meta}${c.prazo ? ` (${c.prazo})` : ''}`)
        .join('\n');

      const novoPlano = [
        current?.plano_terapeutico_atual || '',
        '',
        '--- Última síntese SASI v2.0 ---',
        problemasTxt ? `Problemas: ${problemasTxt}` : '',
        metasTxt ? `Metas ativas:\n${metasTxt}` : '',
      ].filter(Boolean).join('\n').trim();

      const novoSummary: Partial<PatientSummary> = {
        ...(current || {
          id: crypto.randomUUID(),
          paciente_id: paciente.id,
          data_admissao: paciente.data_adm || new Date().toISOString(),
          motivo_admissao: paciente.hd || '',
          dispositivos: [],
          suporte_atual: {},
          created_at: new Date().toISOString(),
        }),
        plano_terapeutico_atual: novoPlano,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mt-6 border-t border-app-border pt-6">
        ...
        <div className="mb-6">
          <div className="text-xs font-bold text-app-text-muted mb-2 flex items-center gap-2">
            <span>VERSÃO TEXTO LIVRE (tradicional)</span>
          </div>

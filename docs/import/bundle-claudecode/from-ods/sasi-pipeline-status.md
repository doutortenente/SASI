...
origin: /home/dr/Downloads/Files_to_claudecode/SASI_codigo_fonte/SASI_Pipeline_Status.ods
ingested: 2026-06-24
kind: ods
---
## Aba `1_SITREP`
...
| Arquivos lib criados | 26 | src/lib/ (types, constants, dictionaries, calculations, guards, scores, alerts) |  |  |  |  |
...
| 1 | Captura no leito | AppSheet app mobile · Google Form | Foto do prontuário · Campos preenchidos manualmente (PAM, lactato, DU, FiO2, doses DVA) | Linha em Google Sheets (timestamp, paciente_id, campo, valor bruto) | PENDENTE |  |
| 2 | Normalização por IA | Gemini 2.5 Flash (via API server-side) | Linha de Sheets bruta (texto OCR / valores inconsistentes em BR-locale) | JSON estruturado {paciente_id, tipo, valor_num, unidade, ts} | PENDENTE |  |
| 3 | Ingestão | Supabase Edge Function (Deno/TypeScript) | Webhook POST de Gemini/AppSheet com JSON validado | INSERT em eventos_clinicos + trigger de recálculo SOFA | PENDENTE — precisa supabase/functions/ocr-ingest/ |  |
| 4 | Persistência | PostgreSQL 15 (Supabase) | INSERT eventos_clinicos(tipo, valor_num, unidade, ts, fonte) | Linha na tabela + trigger sofa_cache_invalidate | SCHEMA INCOMPLETO — tabela eventos_clinicos pendente |  |
| 5 | Recálculo SOFA | Função lib src/lib/scores/sofa.ts (getSOFA) | Patient atualizado (ou leitura da evolucao corrente) | SOFAResult {total, components, detail, warnings, suppressed} | CONCLUÍDO (FASE ALPHA) |  |
| 6 | Propagação Realtime | Supabase Realtime (Postgres CDC + WebSocket) | postgres_changes em evolucoes + eventos_clinicos | Push para clientes conectados — hook useSupabasePatients | HOOK ESBOÇADO (useSupabasePatients.ts existe mas depende do schema) |  |
| 7 | Renderização clínica | React + Tailwind (Dashboard.tsx + PatientCard.tsx) | DashboardRow[] da view vw_dashboard_uti | Card do leito com SOFA delta, alertas cor-codificados, gráfico 72h | USANDO FIREBASE — migração pendente (CHARLIE) |  |
| 8 | Alertas clínicos | src/lib/alerts/engine.ts (runAllAlerts) | Patient + SOFAResult | Alert[] 7 sub-engines (hemo, resp, renal, infecto, metabólico, medicação, sepse) | CONCLUÍDO — falta persistir em alerts_log (FASE BRAVO) |  |
| 9 | Stewardship ATB | View materializada vw_dias_atb_ativo + trigger atb_dias_auto | tabela atbs(data_inicio, data_fim_prevista, intencao) | D-ATB automático por paciente + alerta de suspensão/desescalonamento | PENDENTE — depende de atbs + trigger (FASE BRAVO) |  |
| 10 | Passagem de plantão | Edge Function handoff-pdf.ts + jsPDF | pacientes ativos + últimas evoluções + alertas ativos | PDF one-click pra handoff (TOP 5 automações) | BACKLOG (FASE DELTA) |  |
...
## Aba `4_Kill_Board_Bugs`

| 💀 KILL BOARD — 19 BUGS DA AUDITORIA |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- |
| # | Prioridade | Bug | Subsistema | Status | Correção |  |
| 1 | P0 | SOFA Cardio ignorava peso + diluição em nor/epi | scores/sofa | CORRIGIDO | usa calcDoseInfusao real |  |
| 2 | P0 | SOFA Neuro pontuava GCS baixo sob sedação profunda | scores/sofa | CORRIGIDO | flags sobSedacao + preSedationGCS |  |
| 3 | P0 | Sepsis-3 usava SOFA absoluto em vez de ΔSOFA | scores/sepsis | CORRIGIDO | assessSepsis aceita baseline |  |
| 4 | P0 | hasInfec fraco: ignorava hipotermia/leucopenia; contava profilaxia | scores/sepsis | CORRIGIDO | ATBs com intencao; hipotermia + leucopenia detectadas |  |
| 5 | P0 | SOFA Resp não diferenciava VM ativa vs não-VM | scores/sofa | CORRIGIDO | sofaResp recebe flag onVM |  |
| 6 | P0 | SOFA Renal ignorava DU e TRRC | scores/sofa | CORRIGIDO | cross-ref com calcDiureseEfetiva + flag trrc |  |
| 7 | P1 | parseFloat('37,5') quebrava com vírgula BR | calculations | CORRIGIDO | parseFloatBR aplicado em TODO motor |  |
| 8 | P1 | Plaquetas: heurística frágil de unidade | guards | CORRIGIDO | coercePlaquetas com plaqUnit explícito |  |
| 9 | P1 | FiO2 fração vs percentual (0.4 vs 40) | guards | CORRIGIDO | coerceFiO2Input normaliza 21-100 |  |
| 10 | P1 | Dose absurda silenciosa em calcDoseInfusao | calculations | CORRIGIDO | warnings absurd_high/absurd_low |  |
| 11 | P1 | pam1 vs pam2 ambíguo | types | CORRIGIDO | pam1 = MIN (valor pior) |  |
| 12 | P1 | IDs com Math.random (colisão possível) | utils | CORRIGIDO | crypto.randomUUID + fallback SSR |  |
| 13 | P2 | any em toda assinatura | types | PARCIAL | interface Patient criada — restam componentes React |  |
| 14 | P2 | Zero testes unitários | testing | PARCIAL | 40+ casos em sofa.test.ts — faltam outros módulos |  |
| 15 | P2 | impressao/conduta com slots fixos [4,4,3] | schema | PENDENTE | Schema Supabase permite arrays ilimitados — UI adaptar |  |
| 16 | P2 | diureseHoras hardcoded | UI | PENDENTE | Cosmético — campo livre agora |  |
| 17 | P2 | Culturas sem antibiograma estruturado | types | PARCIAL | Interface Antibiograma existe — UI ainda não usa |  |
| 18 | P2 | Só calcula SOFA — faltam qSOFA/NEWS2/APACHE | scores | PARCIAL | qSOFA implementado — NEWS2 + APACHE pendentes |  |
| 19 | P2 | Defaults de ausculta preenchidos (miniaturizam exame) | UI | PENDENTE | Mudança de UX — manter por ora |  |

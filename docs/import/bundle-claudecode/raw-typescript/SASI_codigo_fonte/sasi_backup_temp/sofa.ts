...
export function sofaResp(
  ...
  if (ratio >= 400) {
    ...
    score = onVM ? 3 : 2;
    if (!onVM) {
      warning = `P/F ${ratioStr} seria SOFA Resp 3 mas paciente não está em VM invasiva — pontuando 2`;
    }
  } else {
    // ratio < 100
    ...
    score = onVM ? 4 : 3;
    if (!onVM) {
      warning = `P/F ${ratioStr} seria SOFA Resp 4 mas paciente não está em VM invasiva — pontuando 3`;
    }
  }
...
export function sofaCoag(
  ...
  return {
    ...
    warning: coerced.warning === 'ambiguous_input'
      ? `Unidade de plaquetas ambígua (${coerced.rawInput}) — confirmar se é /µL ou ×10³/µL`
      : undefined,
  };
}
...
export function sofaNeuro(
  ...
  if (underSedation && !preSedationGCS) {
    return {
      score: 0,
      suppressed: 'Neuro suprimido: paciente sob sedação ativa (GCS reflete droga, não lesão). Documentar GCS pré-sedação para pontuação.',
    };
  }
...
export function sofaRenal(
  ...
  const warning = !isNaN(cr) && duResult.mlPerKgPerHour !== null && scoreByDU > scoreByCr
    ? `DU oligúrico eleva SOFA Renal de ${scoreByCr} para ${finalScore} — IRA por débito!`
    : undefined;

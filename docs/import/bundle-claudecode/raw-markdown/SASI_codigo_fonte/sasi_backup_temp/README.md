---
title: README
origin: /home/dr/Downloads/Files_to_claudecode/SASI_codigo_fonte/sasi_backup_temp/README.md
ingested: 2026-06-24
kind: source-text
---
...
|---|---|---|
| 401 | Token ausente/expirado | Renovar `SUPABASE_ANON_KEY` ou fazer signIn no frontend |
| 400 `target.uti inválido` | Payload veio com UTI errada | Conferir casing: `UTI2`, não `uti2` ou `UTI 2` |
| 400 `Leito X está vago. Envie paciente_upsert` | Primeiro ingest do leito | Incluir `paciente_upsert: { nome: "..." }` no payload |
| 500 `erro inserindo eventos: ...check constraint...` | `tipo` fora da allowlist do schema | Conferir lista de tipos em `references/01-schema-eventos-clinicos.md` |
| 500 `erro inserindo evolucao: ... JSON invalid` | JSONB malformado em `resp`/`hemo`/etc. | Validar JSON no cliente antes de enviar |

---

## ⚔️ Próximos passos

1. **Testes**: criar `__tests__/ocr-ingest.test.ts` com Deno test para cada caso do `troubleshooting`.
2. **Rate limit**: middleware pra limitar 60 req/min por usuário (prevenir spam de OCR).
3. **Audit trail**: segunda function `ocr-ingest-history` que guarda o payload cru em `ingest_audit_log` pra reconstrução forense.
4. **Delta detection**: se evento chega com `tipo='sofa_total'` e é diferente do anterior em >2 pontos → dispara `alerts_log` (BUG do Sepsis-3, handoff item FASE DELTA).

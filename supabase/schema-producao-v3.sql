...
insert into public.evento_tipo_ref (codigo, categoria, rotulo, unidade_padrao, faixa_min, faixa_max, loinc_code, ordem)
values ('pa_sys', 'vital', 'PA sistólica', 'mmHg', 40, 280, '8480-6', 10),
       ...
       ('custom', 'outro', 'Livre — eco/hemodinâmica/imagem/cultura via valor_json {dominio,subtipo,unidade}', '', null,
        null, null, 99)
...
create or replace function public.fn_alert_hash(p_paciente_id uuid, p_tipo text, p_payload jsonb)
  returns text
  language plpgsql
  stable set search_path to 'public','extensions','pg_catalog' as
$$
begin
  return encode(extensions.digest(
                  p_paciente_id::text || '|' || p_tipo || '|' || coalesce(p_payload::text, '') || '|' ||
...
$$
declare
  r        record; best_rank int := 0; v_rank int;
...
begin
  ...
  for r in select * from alert_rules where ativo and tipo_evento = new.tipo
    loop
  ...
  v_hash :=
    fn_alert_hash(new.paciente_id, b_rotulo, jsonb_build_object('valor', new.valor_num, 'tipo_evento', new.tipo));
  ...
  values (new.paciente_id, new.id, new.user_id, b_rotulo, b_sev::public.severidade_alerta_enum,
          ...
          jsonb_build_object('valor', new.valor_num, 'tipo_evento', new.tipo, 'unidade', new.unidade, 'regra', b_id,
                             'fonte', b_fonte),
          v_hash, false)
...
$$
declare
  prev_valor numeric; prev_ts timestamptz; r record; v_delta numeric; v_ratio numeric; v_gap_h numeric;
...
begin
  ...
  for r in select * from trend_rules where ativo and tipo_evento = new.tipo
    loop
      v_hit := case r.modo
  ...
  insert into alerts_log (paciente_id, evento_id, user_id, tipo, severidade, mensagem, payload, hash_key, acked)
  values (new.paciente_id, new.id, new.user_id, b_rotulo, b_sev::public.severidade_alerta_enum,
          replace(replace(replace(b_msg, '{v}', new.valor_num::text), '{prev}', prev_valor::text), '{d}',
                  round(v_delta, 2)::text),
          jsonb_build_object('valor', new.valor_num, 'anterior', prev_valor, 'delta', v_delta, 'gap_horas',
                             round(v_gap_h, 1), 'tipo_evento', new.tipo, 'regra', b_id, 'fonte', b_fonte),
          v_hash, false)
  on conflict (hash_key) do nothing;
  return new;
end;
...
create or replace function public.match_memorias(query_embedding extensions.vector, match_threshold double precision,
                                                 match_count integer)
  returns table
          (
            id         bigint,
  ...
  language sql
  stable set search_path to 'public','extensions','pg_catalog'
as
$$
...
create or replace function public.save_ficha(
  p_paciente_id uuid, p_pac jsonb, p_evol jsonb, p_evolucao_id uuid, p_plantao text,
  p_pendencias jsonb default '[]'::jsonb)
  returns uuid
  language plpgsql
  security definer set search_path to 'public','pg_catalog' as
$$
declare
  v_evol_id uuid; v_uid uuid := auth.uid(); pend jsonb;
begin
  update pacientes
  set nome      = coalesce(p_pac ->> 'nome', nome),
      ...
      gravidade = coalesce((p_pac ->> 'gravidade')::public.gravidade_enum, gravidade),
      data_adm  = coalesce(nullif(p_pac ->> 'data_adm', '')::date, data_adm)
  where id = p_paciente_id
    and (user_id is null or user_id = v_uid); -- guarda de dono
  ...
  if p_evolucao_id is not null then
    update evolucoes
    set neuro=coalesce(p_evol -> 'neuro', '{}'::jsonb),
    ...
    insert into evolucoes (paciente_id, user_id, data_evolucao, plantao, neuro, resp, hemo, tgi, renal, hemato, infecto,
                           dvas, sedativos, impressao, conduta, problemas_ativos, condutas_sistemas, riscos,
                           sofa_snapshot)
    values (p_paciente_id, v_uid, now(), p_plantao::public.plantao_enum,
            coalesce(p_evol -> 'neuro', '{}'::jsonb), coalesce(p_evol -> 'resp', '{}'::jsonb),
            coalesce(p_evol -> 'hemo', '{}'::jsonb),
            coalesce(p_evol -> 'tgi', '{}'::jsonb), coalesce(p_evol -> 'renal', '{}'::jsonb),
            coalesce(p_evol -> 'hemato', '{}'::jsonb),
  ...
  for pend in select * from jsonb_array_elements(coalesce(p_pendencias, '[]'::jsonb))
    loop
      if coalesce(pend ->> 'id', '') <> '' then
        update pendencias
        set tarefa=pend ->> 'tarefa',
...
create trigger trg_updated_at
  ...
  on public.pacientes
  for each row
execute function public.fn_updated_at();
create trigger trg_updated_at
  before update
  on public.evolucoes
  for each row
...
create trigger trg_sync_severidade_visual
  ...
  on public.pacientes
  for each row
execute function public.sync_severidade_visual();
...
create trigger trg_autoflag_lowconf
  before insert
  on public.eventos_clinicos
  for each row
execute function public.fn_autoflag_lowconf();
create trigger trg_eval_alert
  after insert
  on public.eventos_clinicos
  for each row
execute function public.fn_eval_alert();
create trigger trg_eval_trend
  after insert
  on public.eventos_clinicos
  for each row
execute function public.fn_eval_trend();
...
alter table public.trend_rules
  enable row level security;
alter table public.evento_tipo_ref
  enable row level security;
...
with ultima_evol as (select distinct on (e.paciente_id) e.paciente_id,
                                                        e.id            as evolucao_id,
     ...
     sofa_24h_atras as (select distinct on (ec.paciente_id) ec.paciente_id, ec.valor_num as sofa_total_24h
                        from eventos_clinicos ec
                        where ec.tipo = 'sofa_total'
                          and ec.ts <= (now() - interval '24 hour')
                        order by ec.paciente_id, ec.ts desc),
     pend_abertas as (select paciente_id, count(*)::int as pendencias_abertas
                      from pendencias
...
select p.id                                              as paciente_id,
       ...
       u.dvas,
       u.sedativos,
       (u.sofa_total::numeric - s24.sofa_total_24h)::int as delta_sofa_24h,
...
select paciente_id,
       ...
       case
         when ((current_date - data_inicio) + 1) >= 14 then 'critical'
...
from eventos_clinicos
where tipo = 'sofa_total'
  and ts >= now() - interval '72 hour'

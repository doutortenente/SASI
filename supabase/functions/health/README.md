...
- **Auth**: modo `publishable` — exige a chave publicável (`sb_publishable_...`) no header. Por isso o `config.toml`
  desliga a verificação de JWT da plataforma para esta função (`verify_jwt = false`): a chave publicável não é um JWT;
  quem valida é o próprio SDK.

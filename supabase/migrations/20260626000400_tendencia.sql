...
insert into public.trend_rules (tipo_evento, modo, limiar, janela_max_horas, severidade, rotulo, mensagem, fonte, ordem)
values ('cr', 'subida_abs', 0.3, 48, 'warning', 'aki_cr_abs', 'AKI: creatinina subiu {d} mg/dL (de {prev} para {v})',
        'KDIGO / AKI-sepse doi:10.1038/s41581-023-00683-3', 10),
       ('cr', 'subida_rel', 1.5, 168, 'critical', 'aki_cr_rel', 'AKI: creatinina {v} = 1.5x o basal {prev}',
        'KDIGO / AKI-sepse doi:10.1038/s41581-023-00683-3', 11),
       ('gcs', 'queda_abs', 2, 24, 'warning', 'gcs_queda', 'Queda de GCS: {prev} para {v}',
        'ERC/ESICM Post-Resus 2025 doi:10.1016/j.resuscitation.2025.110809', 20)
...
where e.valor_num is not null
  and not e.requires_review
  and coalesce(e.confidence, 1) >= 0.7

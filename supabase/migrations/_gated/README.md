...
O Supabase (e o `supabase db push`) só aplica os `.sql` que estão **direto** em `supabase/migrations/`. Subpastas como
esta (`_gated/`) e `_archive/` são **ignoradas** de propósito.
...
Regra de ouro (PLANO §4): remover a `dev_bypass` antes de ter login de verdade = banco invisível pro app. Copie o
conteúdo no SQL Editor do Supabase, na branch, e rode manualmente.

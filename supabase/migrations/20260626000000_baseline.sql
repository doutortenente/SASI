...
CREATE OR REPLACE FUNCTION "public"."match_memorias"("query_embedding" "public"."vector",
...
BEGIN
  FOR cmd IN
    ...
    LOOP
      IF cmd.schema_name IS NOT NULL AND cmd.schema_name IN ('public') AND
         cmd.schema_name NOT IN ('pg_catalog', 'information_schema') AND cmd.schema_name NOT LIKE 'pg_toast%' AND
         cmd.schema_name NOT LIKE 'pg_temp%' THEN
        BEGIN
...
(
  ...
  CONSTRAINT "alerts_log_severidade_check" CHECK (("severidade" = ANY
...
(
  ...
  CONSTRAINT "atbs_intencao_check" CHECK (("intencao" = ANY
                                           (ARRAY ['empirica'::"text", 'dirigida'::"text", 'profilatica'::"text"]))),
  CONSTRAINT "atbs_via_check" CHECK (("via" = ANY
                                      (ARRAY ['EV'::"text", 'VO'::"text", 'IM'::"text", 'SC'::"text", 'SNE'::"text", 'SNG'::"text", 'IT'::"text", 'Tópico'::"text"]))),
...
(
  ...
  CONSTRAINT "culturas_material_check" CHECK (("material" = ANY
                                               (ARRAY ['hemocultura'::"text", 'urocultura'::"text", 'aspirado_traqueal'::"text", 'lavado_bal'::"text", 'lcr'::"text", 'secrecao_ferida'::"text", 'liquido_peritoneal'::"text", 'liquido_pleural'::"text", 'outro'::"text"])))
);
...
(
  ...
  CONSTRAINT "eventos_clinicos_fonte_check" CHECK (("fonte" = ANY
                                                    (ARRAY ['manual'::"text", 'gemini_ocr'::"text", 'claude_ocr'::"text", 'appsheet'::"text", 'auto_trigger'::"text", 'edge_function'::"text", 'api_import'::"text"]))),
  CONSTRAINT "eventos_clinicos_tipo_check" CHECK (("tipo" = ANY
...
(
  ...
  CONSTRAINT "evolucoes_plantao_check" CHECK (("plantao" = ANY
                                               (ARRAY ['manha'::"text", 'tarde'::"text", 'noite'::"text", 'plantao_24h'::"text"]))),
...
(
  ...
  "imc"                numeric GENERATED ALWAYS AS ("round"(
    ("peso" / (("altura" / (100)::numeric) * ("altura" / (100)::numeric))), 1)) STORED,
  CONSTRAINT "pacientes_altura_check" CHECK ((("altura" >= (30)::numeric) AND ("altura" <= (250)::numeric))),
  CONSTRAINT "pacientes_gravidade_check" CHECK (("gravidade" = ANY
                                                 (ARRAY ['estavel'::"text", 'moderado'::"text", 'grave'::"text", 'critico'::"text", 'obito'::"text"]))),
  CONSTRAINT "pacientes_idade_check" CHECK ((("idade" >= 0) AND ("idade" <= 130))),
  CONSTRAINT "pacientes_isolation_check" CHECK (("isolation" = ANY
                                                 (ARRAY ['none'::"text", 'contact'::"text", 'droplet'::"text", 'aerosol'::"text"]))),
  ...
  CONSTRAINT "pacientes_severidade_visual_check" CHECK (("severidade_visual" = ANY
                                                         (ARRAY ['red'::"text", 'yellow'::"text", 'green'::"text"]))),
  CONSTRAINT "pacientes_sofa_baseline_check" CHECK ((("sofa_baseline" >= 0) AND ("sofa_baseline" <= 24))),
  CONSTRAINT "pacientes_status_leito_check" CHECK (("status_leito" = ANY
                                                    (ARRAY ['ativo'::"text", 'alta'::"text", 'obito'::"text", 'transferencia'::"text"]))),
...
WITH "ultima_evol" AS (SELECT DISTINCT ON ("e"."paciente_id") "e"."paciente_id",
     ...
     "sofa_24h_atras" AS (SELECT DISTINCT ON ("ec"."paciente_id") "ec"."paciente_id",
                                                                  "ec"."valor_num" AS "sofa_total_24h"
                          FROM "public"."eventos_clinicos" "ec"
                          WHERE (("ec"."tipo" = 'sofa_total'::"text") AND
                                 ("ec"."ts" <= ("now"() - '24:00:00'::interval)))
                          ORDER BY "ec"."paciente_id", "ec"."ts" DESC),
     "pend_abertas" AS (SELECT "pendencias"."paciente_id",
...
CREATE OR REPLACE TRIGGER "trg_sofa_cache_invalidate"
  BEFORE UPDATE
  ON "public"."evolucoes"
  FOR EACH ROW
...
CREATE OR REPLACE TRIGGER "trg_sync_severidade_visual"
  BEFORE UPDATE
  ON "public"."pacientes"
  FOR EACH ROW
...
CREATE POLICY "alerts_all_own" ON "public"."alerts_log" USING ((EXISTS (SELECT 1
                                                                        FROM "public"."pacientes" "p"
                                                                        WHERE (("p"."id" = "alerts_log"."paciente_id") AND
                                                                               ("p"."user_id" = "auth"."uid"()))))) WITH CHECK ((EXISTS (SELECT 1
                                                                                                                                         FROM "public"."pacientes" "p"
                                                                                                                                         WHERE (("p"."id" = "alerts_log"."paciente_id") AND
                                                                                                                                                ("p"."user_id" = "auth"."uid"())))));
...
CREATE POLICY "antibiograma_all_own" ON "public"."antibiograma" USING ((EXISTS (SELECT 1
                                                                                ...
                                                                                WHERE (("c"."id" = "antibiograma"."cultura_id") AND
                                                                                       ("p"."user_id" = "auth"."uid"()))))) WITH CHECK ((EXISTS (SELECT 1
                                                                                                                                                 FROM ("public"."culturas" "c"
                                                                                                                                                   JOIN "public"."pacientes" "p"
                                                                                                                                                       ON (("p"."id" = "c"."paciente_id")))
...
CREATE POLICY "atbs_all_own" ON "public"."atbs" USING ((EXISTS (SELECT 1
                                                                FROM "public"."pacientes" "p"
                                                                WHERE (("p"."id" = "atbs"."paciente_id") AND
                                                                       ("p"."user_id" = "auth"."uid"()))))) WITH CHECK ((EXISTS (SELECT 1
                                                                                                                                 FROM "public"."pacientes" "p"
                                                                                                                                 WHERE (("p"."id" = "atbs"."paciente_id") AND
                                                                                                                                        ("p"."user_id" = "auth"."uid"())))));
...
CREATE POLICY "culturas_all_own" ON "public"."culturas" USING ((EXISTS (SELECT 1
                                                                        FROM "public"."pacientes" "p"
                                                                        WHERE (("p"."id" = "culturas"."paciente_id") AND
                                                                               ("p"."user_id" = "auth"."uid"()))))) WITH CHECK ((EXISTS (SELECT 1
                                                                                                                                         FROM "public"."pacientes" "p"
                                                                                                                                         WHERE (("p"."id" = "culturas"."paciente_id") AND
                                                                                                                                                ("p"."user_id" = "auth"."uid"())))));
...
CREATE POLICY "eventos_all_own" ON "public"."eventos_clinicos" USING ((EXISTS (SELECT 1
                                                                               FROM "public"."pacientes" "p"
                                                                               WHERE (("p"."id" = "eventos_clinicos"."paciente_id") AND
                                                                                      ("p"."user_id" = "auth"."uid"()))))) WITH CHECK ((EXISTS (SELECT 1
                                                                                                                                                FROM "public"."pacientes" "p"
                                                                                                                                                WHERE (("p"."id" = "eventos_clinicos"."paciente_id") AND
                                                                                                                                                       ("p"."user_id" = "auth"."uid"())))));
...
CREATE POLICY "evolucoes_all_own" ON "public"."evolucoes" USING ((EXISTS (SELECT 1
                                                                          FROM "public"."pacientes" "p"
                                                                          WHERE (("p"."id" = "evolucoes"."paciente_id") AND
                                                                                 ("p"."user_id" = "auth"."uid"()))))) WITH CHECK ((EXISTS (SELECT 1
                                                                                                                                           FROM "public"."pacientes" "p"
                                                                                                                                           WHERE (("p"."id" = "evolucoes"."paciente_id") AND
                                                                                                                                                  ("p"."user_id" = "auth"."uid"())))));
...
CREATE POLICY "pendencias_all_own" ON "public"."pendencias" USING ((EXISTS (SELECT 1
                                                                            FROM "public"."pacientes" "p"
                                                                            WHERE (("p"."id" = "pendencias"."paciente_id") AND
                                                                                   ("p"."user_id" = "auth"."uid"()))))) WITH CHECK ((EXISTS (SELECT 1
                                                                                                                                             FROM "public"."pacientes" "p"
                                                                                                                                             WHERE (("p"."id" = "pendencias"."paciente_id") AND
                                                                                                                                                    ("p"."user_id" = "auth"."uid"())))));

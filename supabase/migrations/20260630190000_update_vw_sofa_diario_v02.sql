...
with pf_worst as (select distinct on (paciente_id, ts::date) paciente_id,
                                                             ts::date                                  as dia,
                                                             valor_num                                 as pf,
     ...
     score as (select j.*,
                      ...
                      case
                        when plaq is null then null
                        when plaq >= 150 then 0
                        when plaq >= 100 then 1
                        when plaq >= 50 then 2
                        when plaq >= 20 then 3
                        else 4 end                                         as s_coag,
                      case
                        when bb is null then null
                        when bb < 1.2 then 0
                        when bb < 2.0 then 1
                        when bb < 6.0 then 2
                        when bb < 12.0 then 3
                        else 4 end                                         as s_liver,
                      case
                        when pam is null and nor is null and adr is null and dopa is null and dobuta is null then null
                      ...
                      case
                        ...
                        when gcs >= 10 then 2
                        when gcs >= 6 then 3
                        else 4 end                                         as s_neuro,
                      case
                        when cr is null and diurese_h is null then null
                        ...
                        when cr is not null then 0
                        else null end                                      as s_renal
               from j)
select paciente_id,
       dia,
       s_resp,
       ...
       (s_resp is not null)::int + (s_coag is not null)::int + (s_liver is not null)::int
         + (s_cardio is not null)::int + (s_neuro is not null)::int +
       (s_renal is not null)::int                                                                as componentes_presentes,
       array_remove(array [
